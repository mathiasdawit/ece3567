...
//
// Parameters:
// - myUart_Instance:  Let's you select which UART should receive the buffer;
...
//                     Note that receiving a carriage return or line-feed will
...
void Communications_Link(void)
{
    ...
    {
        myUart_writeBuf( BACKCHANNEL_UART, "ECE 3567 Microcontroller Lab", NULL, CRLF );
        myUart_writeBuf( BACKCHANNEL_UART, "Please enter a Command Code: ", NULL, CRLF );
        EUSCI_A_UART_clearInterrupt( EUSCI_A1, EUSCI_A_UART_RECEIVE_INTERRUPT);
        EUSCI_A_UART_enableInterrupt( EUSCI_A1, EUSCI_A_UART_RECEIVE_INTERRUPT);
    }
}
